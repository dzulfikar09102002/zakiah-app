    ...
    return (
        ...
        >
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle>
                        {alertState.delete ? 'Hapus Jenis Pesanan' : 'Pulihkan Jenis Pesanan'}
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                        Apakah anda yakin ingin {alertState.delete ? 'menghapus' : 'memulihkan'} data ini?
                    </AlertDialogDescription>
                </AlertDialogHeader>
                        ...
                        onClick={() => {
                            const options = {
                                ...
                                onError: (errors: any) => {
                                    toast.error(
                                        alertState.delete
                                            ? 'Gagal menghapus Jenis Pesanan'
                                            : 'Gagal memulihkan Jenis Pesanan'
                                    );
                                ...
                                onSuccess: () => {
                                    toast.success(
                                        alertState.delete
                                            ? 'Jenis Pesanan berhasil dihapus'
                                            : 'Jenis Pesanan berhasil dipulihkan'
                                    );
                                },
                            ...
                            alertState.delete
                                ? router.delete(
                                    ordertypes.destroy(alertState.dataId).url,
                                    options
                                )
                                : router.post(
                                    ordertypes.restore(alertState.dataId).url,
                                    {},
                                    options
                                );
                        }}
