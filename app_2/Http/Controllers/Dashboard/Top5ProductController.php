...
class Top5ProductController extends Controller
{
    /**
    ...
    public function index(IndexTop5ProductRequest $request)
    {
        ...
        $datas = SaleTransactionDetail::select('product_id')
            ->select('product_name', DB::raw('sum(total_line_amount) as total_line_amount'), DB::raw('sum(quantity) as quantity'))
